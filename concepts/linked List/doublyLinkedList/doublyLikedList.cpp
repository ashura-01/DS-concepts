...
void deleteFirst()
{
    if(root==nullptr) return;  // nothing to delete
...
{
    if(root==nullptr) return; // no mid in empty list
    ...
    node *mid = slow;
    if(mid->next) mid->next->prev = temp;
    temp->next = mid->next;
    mid-> next = temp;
    temp->prev = mid;

    if(mid==tail) tail=temp;
}
...
void insertByValueBefore(int target,int value)
{
    ...
    if(curr->next==nullptr) return; // target not found
...
void insertByPositionAfter(int position,int value)
{
    ...
    if(curr==nullptr) return; // invalid position
...
void insertByPositionBefore(int position,int value)
{
    ...
    if(curr==nullptr || curr->next==nullptr) return; // invalid position
...
void delmid()
{
    if(root==nullptr || root==tail) return; // empty or single node
